...
| `ch5-2` | lands on master, and each layer adds only what it owns | **Three layers, and the boundaries are the design rather than an accident of history.** `master` holds the source — `LICENSE`, `README.md`, `doc/`, the templates, the code. `upstream/latest` adds **nothing**, and that is its job rather than a gap. **It carried a Python deliverable until 2026-08-17 and that deliverable was retired**: nothing ever built a wheel, `debian/rules` does not select pybuild, the built `.deb` has no dist-packages, and almost everything this package *is* — a conffile, a netfilter-persistent plugin, templates under `/usr/share`, a manpage — lives where a wheel cannot put it. The branch survives because `debian/gbp.conf` names it as the upstream branch: the orig tarball is built from it and pristine-tar regenerates that tarball byte-identically, so its job is to BE the source at a tag. A layer that adds nothing is what that job wants, and it makes the boundary a stronger claim rather than a weaker one — anything on that branch and not on master ships in a tarball the source cannot account for. `debian/latest` adds `debian/`. **Master carrying none of it is the point** — the source moves without a packaging decision attached to every change — and a drill asserts both directions, that `debian/` never appears on master and that `upstream/latest` adds the declared set and nothing else. Collapsing the boundary is not hypothetical: doing it gave the licence file two names and two homes, and it is what a later reader will do again unless something refuses |
...
- **ch5-U5 — the manpage is written by hand and drifts, and the thing that would have generated it
  ...
  measurable: on 2026-08-17 the manpage still carried the version stamp of two releases earlier and
  described `add-service` as writing a template, which it had not done since chapter 8 — corrected
  by hand, which is the correction that will be needed again. **The idea does not need hatch**: the
  parser is the authority for what the options are, and a script in `tools/` run during the cut
