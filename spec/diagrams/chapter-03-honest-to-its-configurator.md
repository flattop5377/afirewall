...
anything at all: `a play` wrote `inbound.tor` for years, a key this package has never
had. The task succeeded every time. The line persisted, survived every reload, and opened nothing.
...
| `ch3-3` | is this a real run, or is it being asked? | **A configuration manager asks before it acts, and a tool that cannot be asked makes that mode a lie.** This deployment has already paid for it: a `command:` under ansible's `--check` does not run, returns rc 0 with empty stdout, and does not leave its register undefined — so a play read a fabricated success and concluded an LVM volume existed on a host with no LVM. A dry run has to be a thing the *command* supports, because the alternative is the caller pretending |
...
| `ch3-7` | a converged host is quiet, and a changed one says what changed | **The measure, and it is an operator's experience rather than a property of the code.** A run that reports changes on a host nothing changed on trains the person reading it to stop reading it, and the operator has already lost one signal that way — `ch6-8` in the ansible spec purged a scheduled all-clear for the same reason. Quiet is not cosmetic; it is what makes noise mean something |
...
- **ch3-U3 — nothing here has been measured against a real converge.** The claim that a converged
  ...
  It cannot be taken until `a play` is in the converge at all, which is `ch9-11` in the ansible
  spec. Anchored to `ch3-7`.
